...
type AppView = "LESSONS" | "EXAMS" | "MY_RESULTS" | "FORMULAS" | "QUIZ" | "RESULT" | "ADMIN";
...
export default function App() {
  ...
  const handleSaveStudentProfile = (profile: StudentProfile) => {
    storageService.saveStudentProfile(profile); setStudentProfile(profile); setIsStudentModalOpen(false);
    if (pendingLessonToStart) { startQuizSession(pendingLessonToStart.lesson, pendingLessonToStart.questions, pendingLessonToStart.isRetake); setPendingLessonToStart(null); }
  };

  const startQuizSession = (lesson: Lesson, customQuestions?: Question[], isRetake = false) => {
    if (!studentProfile) { setPendingLessonToStart({ lesson, questions: customQuestions, isRetake }); setIsStudentModalOpen(true); return; }
    const quizQuestions = customQuestions || questions.filter((q) => q.lessonId === lesson.id);
    if (quizQuestions.length === 0) { alert(`Bài học "${lesson.title}" hiện chưa có câu hỏi nào trong ngân hàng.`); return; }
    if (isRetake) storageService.clearActiveDraft(lesson.id);
    setIsActiveLessonRetake(isRetake); setActiveLesson(lesson); setActiveQuizQuestions(quizQuestions); setCurrentView("QUIZ"); window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const handleFinishQuiz = (result: SubmissionResult) => { setLatestResult(result); setSubmissions(storageService.getSubmissions()); setCurrentView("RESULT"); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const handleNavigate = (view: AppView) => { if (view === "ADMIN") { if (isAdminAuthenticated) setCurrentView("ADMIN"); else setIsAdminLoginModalOpen(true); } else setCurrentView(view); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  const handleBulkImportQuestions = (newQuestions: Question[], mode: "MERGE" | "REPLACE") => { const finalQuestions = mode === "REPLACE" ? newQuestions : [...questions, ...newQuestions.filter((q) => !new Set(questions.map(x => x.id)).has(q.id))]; setQuestions(finalQuestions); void storageService.syncQuestions(finalQuestions).catch(console.error); };
  const handleSaveExam = (exam: Exam, newQuestions?: Question[]) => { if (newQuestions?.length) { const filtered = newQuestions.filter(q => !questions.some(e => e.id === q.id)); if (filtered.length) { setQuestions([...questions, ...filtered]); void storageService.syncQuestions(filtered).catch(console.error); } } const updated = exams.some(e => e.id === exam.id) ? exams.map(e => e.id === exam.id ? exam : e) : [exam, ...exams]; setExams(updated); void storageService.syncExams([exam]).catch(console.error); };
  ...
  const handleToggleLockExam = (examId: string) => { const updated = exams.map(e => e.id === examId ? { ...e, isLocked: !e.isLocked } : e); setExams(updated); const exam = updated.find(e => e.id === examId); if (exam) void storageService.syncExams([exam]).catch(console.error); };
  ...
  return <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 selection:bg-sky-500 selection:text-white">
    <NetworkStatusBanner />
    ...
    <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-12">
      ...
      {currentView === "EXAMS" && <PracticeExamsView exams={exams} questions={questions} student={studentProfile} isAdmin={isAdminAuthenticated} onStartExam={(lesson, qs) => startQuizSession(lesson, qs, true)} onOpenStudentModal={() => setIsStudentModalOpen(true)} onOpenAdminExamManager={() => setCurrentView("ADMIN")} />}
