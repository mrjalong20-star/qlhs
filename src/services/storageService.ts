...
function requestHeaders(options:RequestInit = {}){ const headers:Record<string,string>={Accept:"application/json",...authService.headers()}; const studentToken=typeof window!=="undefined"?localStorage.getItem("geo11_student_session_id"):null; if(studentToken && !headers.Authorization) headers.Authorization=`Bearer ${studentToken}`; if(options.body) headers["Content-Type"]="application/json"; Object.assign(headers,(options.headers||{}) as Record<string,string>); return headers; }
...
export const storageService = {
  ...
  clearStudentProfile(){try{localStorage.removeItem(STORAGE_KEYS.STUDENT_PROFILE);localStorage.removeItem("geo11_student_session_id");}catch{}},
