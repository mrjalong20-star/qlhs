...
export function ExamEditorModal({
  ...
  // Lesson Picker state (for 'Từ bài giảng' mode)
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button
                  type="button"
                  onClick={() => { setShowLessonPicker(!showLessonPicker); setPickMode("MANUAL"); }}
                ...
                >
                  ...
                  <span>Từ bài giảng</span>
                </button>
                <button
                  type="button"
                  onClick={() => { setPickMode("WORD_IMPORT"); setShowLessonPicker(false); }}
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-colors cursor-pointer flex items-center gap-1.5 ${
                ...
                <button
                  type="button"
                  onClick={() => { setPickMode("MANUAL"); setShowLessonPicker(false); }}
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-colors cursor-pointer ${
                    pickMode === "MANUAL" && !showLessonPicker
                      ? "bg-indigo-600 text-white border-indigo-600 shadow-xs"
                      : "bg-white text-slate-700 border-slate-200 hover:bg-slate-50"
                  }`}
                >
            ...
            {showLessonPicker && (
              <div className="bg-emerald-50/70 p-5 rounded-2xl border border-emerald-200 space-y-4">
                <div className="flex items-center gap-2 text-emerald-900 font-extrabold text-xs">
                  ...
                  <span>CHỌN BÀI GIẢNG ĐỂ TẠO ĐỀ KIỂM TRA:</span>
                  <span className="text-[10px] font-bold text-emerald-600 ml-2">({pickedLessonIds.length} bài đã chọn)</span>
                </div>

                <p className="text-[11px] text-emerald-700">Chọn bài giảng muốn lấy câu hỏi. Hệ thống sẽ tự động đưa tất cả câu hỏi từ bài đã chọn vào đề thi.</p>
                ...
                <div className="max-h-64 overflow-y-auto space-y-1.5 pr-1">
                  {[...lessons].sort((a, b) => a.lessonNumber - b.lessonNumber).map((lesson) => {
                    ...
                    const isPicked = pickedLessonIds.includes(lesson.id);
                    return (
                      <button
                        key={lesson.id}
                        type="button"
                        onClick={() => {
                          setPickedLessonIds(prev =>
                            isPicked ? prev.filter(id => id !== lesson.id) : [...prev, lesson.id]
                          );
                        }}
                        className={`w-full text-left px-3 py-2.5 rounded-xl text-xs flex items-center gap-3 transition-all border-2 ${
                          isPicked
                            ? "bg-emerald-100 border-emerald-400 text-emerald-900"
                            : "bg-white border-transparent hover:bg-emerald-50 text-slate-700"
                        }`}
                      >
                        ...
                        <span className="font-bold text-slate-400 w-7 shrink-0">{lesson.lessonNumber}</span>
                        <span className="font-semibold truncate flex-1">{lesson.title}</span>
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold shrink-0 ${
                          qCount > 0 ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-400"
                        }`}>{qCount} câu</span>
                        <span className="text-[10px] text-slate-400 shrink-0">{lesson.chapter}</span>
                      </button>
                    );
                  })}
                </div>
                ...
                <div className="flex justify-between items-center pt-2">
                  <button type="button" onClick={() => { setPickedLessonIds(lessons.map(l => l.id)); }} className="text-xs font-bold text-emerald-700 hover:text-emerald-900">
                    Chọn tất cả ({lessons.length} bài)
                  </button>
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setShowLessonPicker(false)} className="px-3 py-2 rounded-xl text-xs font-bold text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 cursor-pointer">
                      Đóng
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        // Auto-select all questions from picked lessons
                        const allQs = questions.filter(q => pickedLessonIds.includes(q.lessonId));
                        setSelectedQuestionIds(allQs.map(q => q.id));
                        setShowLessonPicker(false);
                        setPickMode("MANUAL");
                      }}
                      disabled={pickedLessonIds.length === 0}
                      className="px-4 py-2 rounded-xl text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 cursor-pointer flex items-center gap-1.5"
                    >
                      <CheckCircle2 className="w-3.5 h-3.5" />
                      <span>LẤY {questions.filter(q => pickedLessonIds.includes(q.lessonId)).length} CÂU HỎI VÀO ĐỀ</span>
                    </button>
                  </div>
                </div>
              </div>
