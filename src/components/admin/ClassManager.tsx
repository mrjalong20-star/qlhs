...
export function ClassManager() {
  const [classes, setClasses] = useState<SchoolClass[]>([]); const [name, setName] = useState(""); const [grade, setGrade] = useState<Grade>(6);
  const [loading, setLoading] = useState(false); const [loadingList, setLoadingList] = useState(false); const [message, setMessage] = useState(""); const [messageType, setMessageType] = useState<"success" | "error" | "info">("info");

  async function load() {
    setLoadingList(true);
    try {
      const session = authService.get(); if (!session?.token) throw new Error("Phiên đăng nhập không hợp lệ. Vui lòng đăng nhập lại.");
    ...
    } catch (e: any) { setMessage(e?.message || "Không thể kết nối máy chủ."); setMessageType("error"); }
  ...
  async function createClass() {
    ...
    try {
      const res = await fetch("/api/classes", { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json", ...authService.headers() }, body: JSON.stringify({ name: cleanName, grade }) });
      const raw = await res.text(); let json: any = {}; try { json = raw ? JSON.parse(raw) : {}; } catch {}
      if (!res.ok || !json.success) throw new Error(json.message || raw || `Tạo lớp thất bại (HTTP ${res.status}).`);
      const created = json.data as SchoolClass; const createdClass: SchoolClass = { ...created, grade: Number(created?.grade || grade) as Grade, studentCount: Number(created?.studentCount || 0) };
      setClasses((current) => [createdClass, ...current.filter((c) => c.id !== createdClass.id)]); setName(""); setGrade(6);
      const url = `${window.location.origin}${json.joinUrl || `/?class=${encodeURIComponent(createdClass.id)}`}`; const copied = await copyText(url);
      setMessage(copied ? `Đã tạo lớp "${createdClass.name}" thành công. Link lớp đã được sao chép.` : `Đã tạo lớp "${createdClass.name}" thành công. Link lớp: ${url}`); setMessageType("success"); void load();
    } catch (e: any) { setMessage(e?.message || "Không thể tạo lớp."); setMessageType("error"); }
