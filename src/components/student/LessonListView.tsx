...
import { Lesson, Question, SubmissionResult, StudentProfile, Grade } from "../../types";
...
export function LessonListView({ lessons, questions, submissions, student, onStartLesson, onOpenStudentModal }: LessonListViewProps) {
  ...
  return <div id="lesson-list-view" className="space-y-6">
    ...
    {filteredLessons.length ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">{filteredLessons.map(lesson=>{
      const lessonQuestions=questions.filter(q=>q.lessonId===lesson.id); const hasDraft=Boolean(storageService.getActiveDraft(lesson.id));
      const grade=lesson.grade || 6; const studentGrade=Number(student?.grade || grade);
      const teacherLocked=Boolean(lesson.isLocked);
      const futureGrade=Boolean(student && grade>studentGrade);
      const currentSequentialLocked=Boolean(student && grade===studentGrade && lesson.lessonNumber>currentGradeNextLessonNumber && !completedLessonIds.has(lesson.id));
      const effectiveLocked=teacherLocked || futureGrade || currentSequentialLocked;
      const lockedLesson={...lesson,isLocked:effectiveLocked};
      return <LessonCard key={lesson.id} lesson={lockedLesson} questions={lessonQuestions} submissions={submissions} hasDraft={hasDraft} onStartLesson={onStartLesson}/>;
    })}</div> : <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center"><BookOpen className="w-12 h-12 text-slate-300 mx-auto mb-3"/><h3 className="text-base font-bold text-slate-700">Không tìm thấy bài học</h3><p className="text-xs text-slate-400 mt-1">Hãy đổi khối, học kì hoặc từ khóa.</p></div>}
  </div>;
}
