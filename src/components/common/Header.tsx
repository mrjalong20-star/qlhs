...
import { Compass, BookOpen, FileCheck2, Trophy, Calculator, User, Lock, Menu, X, Sparkles, LogOut } from "lucide-react";
import { StudentProfile, AppConfig } from "../../types";

interface HeaderProps {
  config: AppConfig; studentProfile: StudentProfile | null;
  currentView: "LESSONS" | "EXAMS" | "MY_RESULTS" | "FORMULAS" | "QUIZ" | "RESULT" | "ADMIN";
  onOpenStudentGate: () => void; onOpenAdminLogin: () => void; onStudentLogout: () => void;
  onNavigate: (view: "LESSONS" | "EXAMS" | "MY_RESULTS" | "FORMULAS" | "ADMIN") => void;
  isAdmin: boolean; authSession?: { role: "SUPER_ADMIN" | "TEACHER"; username: string; displayName: string } | null; onLogout: () => void;
}

export function Header({ config, studentProfile, currentView, onOpenStudentGate, onOpenAdminLogin, onStudentLogout, onNavigate, isAdmin, authSession, onLogout }: HeaderProps) {
  ...
  return <header id="main-header" className="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-slate-200 shadow-xs">
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8"><div className="flex items-center justify-between h-16 sm:h-20">
      ...
      <nav id="desktop-nav" className="hidden lg:flex items-center gap-1">
        <button id="nav-tab-lessons" onClick={() => onNavigate("LESSONS")} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${currentView === "LESSONS" ? "bg-sky-50 text-sky-700 font-semibold" : "text-slate-600 hover:bg-slate-100"}`}><BookOpen className="w-4 h-4"/><span>Bài học</span></button>
    ...
    {mobileMenuOpen && <div id="mobile-nav-drawer" className="lg:hidden border-t border-slate-200 py-3 space-y-1 bg-white">
      <button onClick={() => {onNavigate("LESSONS");setMobileMenuOpen(false);}} className="w-full text-left px-4 py-3 rounded-lg text-sm"><BookOpen className="inline w-5 h-5 mr-2 text-sky-600"/>Bài học</button>
